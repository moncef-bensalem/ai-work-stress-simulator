import { Link, useNavigate } from 'react-router-dom'
import { useState } from 'react'
import { GoogleLogin, type CredentialResponse } from '@react-oauth/google'
import { useAuth } from '../contexts/AuthContext'
import { usePreferences } from '../contexts/PreferencesContext'
import { toast } from 'sonner'
import AuthLayout from '../components/layout/AuthLayout'
import Button from '../components/ui/Button'
import FaceCaptureModal from '../components/FaceCaptureModal'
import EmailInput from '../components/auth/EmailInput'
import PasswordInput from '../components/auth/PasswordInput'
import { isStrongPassword, validateEmail } from '../lib/validation'

const GOOGLE_CLIENT_ID = import.meta.env.VITE_GOOGLE_CLIENT_ID as string | undefined

export default function LoginPage() {
  const { login, loginGoogle, loginFace } = useAuth()
  const { t } = usePreferences()
  const navigate = useNavigate()
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [loading, setLoading] = useState(false)
  const [showPassword, setShowPassword] = useState(false)
  const [faceModalOpen, setFaceModalOpen] = useState(false)
  const [emailTouched, setEmailTouched] = useState(false)
  const [passwordTouched, setPasswordTouched] = useState(false)

  const emailValid = validateEmail(email).valid
  const passwordStrong = isStrongPassword(password)

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setEmailTouched(true)
    setPasswordTouched(true)

    if (!emailValid) {
      toast.error(t.login.invalidEmail)
      return
    }
    if (!passwordStrong) {
      toast.error(t.login.weakPasswordHint)
      return
    }

    setLoading(true)
    try {
      await login(email.trim(), password)
      toast.success(t.login.success)
      navigate('/')
    } catch {
      toast.error(t.login.invalidCredentials)
    } finally {
      setLoading(false)
    }
  }

  const handleGoogle = async (response: CredentialResponse) => {
    if (!response.credential) return
    setLoading(true)
    try {
      await loginGoogle(response.credential)
      toast.success(t.login.googleSuccess)
      navigate('/')
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t.login.googleError)
    } finally {
      setLoading(false)
    }
  }

  const handleFaceLogin = async (descriptor: number[]) => {
    setEmailTouched(true)
    if (!emailValid) {
      toast.error(t.login.invalidEmail)
      throw new Error('Invalid email')
    }
    await loginFace(email.trim(), descriptor)
    setFaceModalOpen(false)
    toast.success(t.login.faceSuccess)
    navigate('/')
  }

  return (
    <>
      <AuthLayout
        title={t.login.title}
        subtitle={t.login.subtitle}
        footer={
          <p>
            {t.login.noAccount}{' '}
            <Link to="/register" className="text-violet-400 hover:text-violet-300 font-medium">
              {t.login.register}
            </Link>
          </p>
        }
      >
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          <div className="space-y-2 animate-fade-up stagger-1">
            <label className="text-sm font-medium text-slate-300" htmlFor="login-email">
              {t.login.email}
            </label>
            <EmailInput
              id="login-email"
              value={email}
              onChange={setEmail}
              onBlur={() => setEmailTouched(true)}
              touched={emailTouched}
            />
          </div>

          <div className="space-y-2 animate-fade-up stagger-2">
            <div className="flex justify-between items-center">
              <label className="text-sm font-medium text-slate-300" htmlFor="login-password">
                {t.login.password}
              </label>
              <Link to="/forgot-password" className="text-xs text-cyan-400 hover:text-cyan-300">
                {t.login.forgotPassword}
              </Link>
            </div>
            <PasswordInput
              id="login-password"
              value={password}
              onChange={(v) => {
                setPassword(v)
                if (!passwordTouched && v.length > 0) setPasswordTouched(true)
              }}
              touched={passwordTouched}
              requireStrong
              showToggle
              showPassword={showPassword}
              onToggleShow={() => setShowPassword(!showPassword)}
            />
          </div>

          <Button
            type="submit"
            loading={loading}
            className="w-full animate-fade-up stagger-3"
            disabled={!emailValid || !passwordStrong}
          >
            {t.login.submit}
          </Button>

          <div className="relative py-2 animate-fade-up stagger-3">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-slate-800" />
            </div>
            <div className="relative flex justify-center text-xs">
              <span className="bg-transparent px-3 text-slate-500">{t.login.orContinue}</span>
            </div>
          </div>

          <div className="space-y-3 animate-fade-up stagger-4">
            {GOOGLE_CLIENT_ID ? (
              <div className="flex justify-center">
                <GoogleLogin
                  onSuccess={handleGoogle}
                  onError={() => toast.error(t.login.googleError)}
                  theme="filled_black"
                  size="large"
                  text="continue_with"
                  shape="pill"
                  width="320"
                />
              </div>
            ) : (
              <Button
                type="button"
                variant="ghost"
                className="w-full"
                onClick={() => toast.info(t.login.googleEnvHint)}
              >
                <GoogleIcon /> {t.login.google}
              </Button>
            )}

            <Button
              type="button"
              variant="ghost"
              className="w-full"
              onClick={() => {
                setEmailTouched(true)
                if (!emailValid) {
                  toast.error(t.login.emailRequiredFace)
                  return
                }
                setFaceModalOpen(true)
              }}
              disabled={loading}
            >
              <FaceIcon /> {t.login.face}
            </Button>
          </div>
        </form>
      </AuthLayout>

      <FaceCaptureModal
        open={faceModalOpen}
        mode="login"
        onClose={() => setFaceModalOpen(false)}
        onCapture={handleFaceLogin}
      />
    </>
  )
}

function GoogleIcon() {
  return (
    <svg className="w-5 h-5" viewBox="0 0 24 24" aria-hidden>
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" />
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
    </svg>
  )
}

function FaceIcon() {
  return (
    <svg className="w-5 h-5 text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden>
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10S2 17.523 2 12z" />
    </svg>
  )
}
